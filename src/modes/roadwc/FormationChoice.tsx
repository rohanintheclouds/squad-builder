  ...
  return (
    <div className="relative w-full overflow-hidden rounded-lg" style={{ aspectRatio: '68 / 92', background: 'linear-gradient(180deg,#1a6e3f,#0c3f23)' }}>
      {f.slots.map((s) => (
        <span
          key={s.id}
          className="absolute h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full ring-1 ring-black/30"
...
export default function FormationChoice() {
  const { formationChoices, chooseFormation } = useWcStore()
  return (
